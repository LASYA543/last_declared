...
void travel :: user_login()
{
        int count;
        string username,password,user,pass;
        LOG_INFO("please enter the following details to login");
        LOG_INFO("USERNAME :");
         cin>>username;
        LOG_INFO("PASSWORD :");
        cin>>password;
...
void travel :: user_forgot()
{
 ...
 if(user==search_user)
                ...
                default:
                        LOG_INFO("Sorry, You entered wrong choice. Kindly try again");
                        user_forgot();
        }
}
...
void travel :: user_writedata(){
  ...
  cin >> r.user_name;
  LOG_INFO("ENTER YOUR CONTACT NO.: ");
  cin >> r.user_phone;
  LOG_INFO("ENTER NO. OF TOURIST: ");
  cin >> r.user_people;
  int People = atoi (r.user_people);
  LOG_INFO("ENTER THE DESTINATION CODE: D-");
  cin >> r.user_dcode;
  LOG_INFO("ENTER DATE OF JOURNEY (DD/MM/YY): ");
  cin >> r.user_date;
  LOG_INFO("ENTER PAYMENT METHOD CASH OR CARD: ");
  cin >> r.user_payments;
  LOG_INFO("TOTAL PRICES ");
...
void travel::choosevehicle(){
    ...
    switch(choice)
    {
        ...
        cin >> cabChoice;
        LOG_INFO("Enter no of passengers");
        cin>>passengers;
